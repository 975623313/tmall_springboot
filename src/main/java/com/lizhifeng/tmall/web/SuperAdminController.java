...
//	    	subject.getSession().setAttribute("user", user);
    ...
    @GetMapping("/supers")
    public Page4Navigator<Admin> list(@RequestParam(value = "start", defaultValue = "0") int start, @RequestParam(value = "size", defaultValue = "5") int size) throws Exception {

        start = start<0?0:start;
        Page4Navigator<Admin> page = this.superService.list(start,size,5);
        return page;
    }
    ...
    public Admin get(@PathVariable("id") int id) throws Exception {
        Admin bean=superService.get(id);

        System.out.println(bean);
        return bean;
    }
    @PutMapping("/updateAdmin")
    public Object update(@RequestBody Admin bean) throws Exception {
        superService.update(bean);
        return bean;
    }
